                                    ...
                                    <td>
                                        @if($waste->bankSampah)
                                            <div>
                                                ...
                                                <small class="text-muted">
                                                    <i class="bi bi-geo-alt me-1"></i>{{ $waste->bankSampah->location_name ?? Str::limit($waste->bankSampah->address, 40) }}
                                                </small>
                                            </div>
                                        @else
                                    ...
                                    <td>
                                        <div class="d-flex gap-1 justify-content-center align-items-center">
                                            @if($waste->status === 'pending')
                                                <form action="{{ route('admin.waste.approve', $waste->id) }}" method="POST" class="d-inline" id="approve-form-{{ $waste->id }}">
                                                    @csrf
                                                    <button type="button" 
                                                ...
                                                <form action="{{ route('admin.waste.reject', $waste->id) }}" method="POST" class="d-inline" id="reject-form-{{ $waste->id }}">
                                                    @csrf
                                                    <button type="button" 
                            ...
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <div class="flex-grow-1">
                                        ...
                                        <div class="d-flex flex-wrap gap-2 text-muted small mb-2">
                                            <span><i class="bi bi-calendar3 me-1"></i>{{ $waste->created_at->format('d/m/Y') }}</span>
                                            <span><i class="bi bi-clock me-1"></i>{{ $waste->created_at->format('H:i') }}</span>
                                            <span><i class="bi bi-box-seam me-1"></i>{{ number_format($waste->unit === 'gram' ? $waste->amount / 1000 : $waste->amount, 2) }} kg</span>
                                ...
                                @if($waste->status === 'pending')
                                    <div class="d-flex gap-1 justify-content-end mt-3">
                                        <form action="{{ route('admin.waste.approve', $waste->id) }}" method="POST" class="d-inline" id="approve-form-mobile-{{ $waste->id }}">
                                            @csrf
                                            <button type="button" 
                                                    class="btn btn-sm btn-success btn-action" 
                                                    title="Setujui"
                                        ...
                                        <form action="{{ route('admin.waste.reject', $waste->id) }}" method="POST" class="d-inline" id="reject-form-mobile-{{ $waste->id }}">
                                            @csrf
                                            <button type="button" 
                                                    class="btn btn-sm btn-danger btn-action" 
                                                    title="Tolak"
